...
function Editscreen(props: {
 ...
 function Select(id: number) {
  try {
   ...
   Vendor.current.value = filament['vendor']
   Color.current.value = filament['color']
   Type.current.value = filament['type']
   Price.current.value = filament['price']
   Weight.current.value = filament['weight']
  } catch (error) {
   console.error(error)
  }
 }
 ...
 async function EditFilament() {
  if (!vendor || !color || !type || !price || !weight) {
   ...
   const res = await index.invoke('edit_filament', {
    id: selected,
    vendor: vendor,
    color: color,
    type: type,
 ...
 return (
  <div
   ...
   <div className="flex flex-col items-center">
    <h1 className="text-4xl text-white font-bold">Edit Filament</h1>
    <div className="w-full grid grid-cols-2 gap-4 mx px-4">
     <div className="flex flex-col ">
      ...
      <Button color="primary" type="button" onClick={() => EditFilament()} size="lg">
       Edit
      </Button>
     </div>

     <div className="w-full">
      <div className="bg-slate-300 text-black inline-grid grid-cols-3 gap-4 w-full text-sm">
       <div>Vendor</div>
       <div>Color</div>
       <div>Type</div>
      </div>
       ...
       (
        ...
        return (
         <div
          ...
          className="inline-grid grid-cols-3 gap-4 hover:bg-white/30 w-full text-sm"
          onClick={() => Select(key)}>
